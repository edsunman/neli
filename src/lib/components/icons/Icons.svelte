<script lang="ts" module>
	export {
		addIcon,
		aspectLandscape,
		aspectPortrait,
		aspectSquare,
		audioIcon,
		backIcon,
		backArrowIcon,
		checkIcon,
		copyIcon,
		cropIcon,
		deleteIcon,
		easeInIcon,
		easeOutIcon,
		exportIcon,
		fileIcon,
		fileNewIcon,
		fileOpenIcon,
		filmIcon,
		folderIcon,
		forwardArrowIcon,
		forwardIcon,
		handIcon,
		helpIcon,
		imageIcon,
		importIcon,
		infoIcon,
		inPointIcon,
		justifyLeftIcon,
		justifyCenterIcon,
		justifyRightIcon,
		keyframeIcon,
		linkIcon,
		logoIcon,
		mouseIcon,
		moveIcon,
		opacityIcon,
		outPointIcon,
		paletteIcon,
		pauseIcon,
		playIcon,
		pointerIcon,
		presetsIcon,
		redoIcon,
		scaleToFitIcon,
		scaleToFillIcon,
		scissorsIcon,
		seekIcon,
		settingsIcon,
		speakerIcon,
		spinningIcon,
		testIcon,
		textIcon,
		undoIcon,
		warningIcon,
		zoomInIcon,
		zoomOutIcon
	};
</script>

{#snippet addIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M17 12L7 12M12 17L12 7" /><!-- <circle cx="12" cy="12" r="10" /> -->
	</svg>
{/snippet}

{#snippet aspectLandscape(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M200-240q-33 0-56.5-23.5T120-320v-320q0-33 23.5-56.5T200-720h560q33 0 56.5 23.5T840-640v320q0 33-23.5 56.5T760-240H200Zm0-80h560v-320H200v320Zm0 0v-320 320Z"
		/>
	</svg>
{/snippet}

{#snippet aspectPortrait(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M240-760q-0-33 23.5-56.5T320-840h320q33-0 56.5 23.5T720-760v560q0 33-23.5 56.5T640-120h-320q-33 0-56.5-23.5T240-200V-760Zm80-0v560h320V-760h-320Zm0 0h320-320Z"
		/>
	</svg>
{/snippet}

{#snippet aspectSquare(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm0-80h560v-560H200v560Zm0 0v-560 560Z"
		/>
	</svg>
{/snippet}

{#snippet audioIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12 7L12 17" /> <path d="M15 10L15 14" /> <path d="M18 8L18 16" />
		<path d="M21 13L21 11" /> <path d="M9 4L9 20" /> <path d="M6 9L6 15" /> <path d="M3 13L3 11" />
	</svg>
{/snippet}

{#snippet backIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 240 240"
		fill="currentColor"
	>
		<path
			d="M26.18,96.87l80.42-46.43c17.8-10.28,40.06,2.57,40.06,23.13v92.86c0,20.56-22.25,33.4-40.06,23.13l-80.42-46.43c-17.8-10.28-17.8-35.97,0-46.25Z"
		/><rect
			x="178.92"
			y="46"
			width="45.61"
			height="148"
			rx="22.8"
			ry="22.8"
			transform="translate(403.46 240) rotate(-180)"
		/>
	</svg>
{/snippet}

{#snippet backArrowIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path d="M400-80 0-480l400-400 71 71-329 329 329 329-71 71Z" />
	</svg>
{/snippet}

{#snippet checkIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z" />
	</svg>
{/snippet}

{#snippet cropIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M680-40v-160H280q-33 0-56.5-23.5T200-280v-400H40v-80h160v-160h80v640h640v80H760v160h-80Zm0-320v-320H360v-80h320q33 0 56.5 23.5T760-680v320h-80Z"
		/>
	</svg>
{/snippet}

{#snippet copyIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<rect width="12" height="14" x="8" y="7" /> <polyline points="16 3 4 3 4 17" />
	</svg>
{/snippet}

{#snippet deleteIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"
		/>
	</svg>
{/snippet}

{#snippet easeInIcon(className: string)}
	<svg
		class={className}
		version="1.1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			class="cls-1"
			d="M2.2,16.67c4.37,0,5.78-3.2,8.07-7.13,2.06-3.53,3.63-6.23,7.43-6.23"
		/><line class="cls-1" x1="6.68" y1="3.3" x2="5.06" y2="3.3" /><circle
			class="cls-1"
			cx="3.08"
			cy="3.3"
			r="1.98"
		/><line class="cls-1" x1="10.81" y1="3.3" x2="9.85" y2="3.3" />
	</svg>
{/snippet}

{#snippet easeOutIcon(className: string)}
	<svg
		class={className}
		version="1.1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			class="cls-1"
			d="M2.2,16.67c4.37,0,5.78-3.2,8.07-7.13,2.06-3.53,3.63-6.23,7.43-6.23"
		/><line class="cls-1" x1="13.32" y1="16.67" x2="14.93" y2="16.67" /><circle
			class="cls-1"
			cx="16.91"
			cy="16.67"
			r="1.98"
		/><line class="cls-1" x1="9.18" y1="16.67" x2="10.15" y2="16.67" />
	</svg>
{/snippet}

{#snippet exportIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12,3 L12,16" /> <polyline points="7 12 12 17 17 12" /> <path d="M20,21 L4,21" />
	</svg>
{/snippet}

{#snippet fileIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path stroke-linecap="round" d="M13 3v6h6" /> <path d="M13 3l6 6v12H5V3z" />
	</svg>
{/snippet}

{#snippet fileNewIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M440-240h80v-120h120v-80H520v-120h-80v120H320v80h120v120ZM240-80q-33 0-56.5-23.5T160-160v-640q0-33 23.5-56.5T240-880h320l240 240v480q0 33-23.5 56.5T720-80H240Zm280-520v-200H240v640h480v-440H520ZM240-800v200-200 640-640Z"
		/>
	</svg>
{/snippet}

{#snippet fileOpenIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M240-80q-33 0-56.5-23.5T160-160v-640q0-33 23.5-56.5T240-880h320l240 240v240h-80v-200H520v-200H240v640h360v80H240Zm638 15L760-183v89h-80v-226h226v80h-90l118 118-56 57Zm-638-95v-640 640Z"
		/>
	</svg>
{/snippet}

{#snippet filmIcon(className: string)}
	<svg
		class={className}
		version="1.1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		fill="currentColor"
	>
		<path
			class="st0"
			d="M3.9,2.8l1.4,2.8c0,.2.2.3.4.4.2,0,.3.1.5.1.4,0,.6-.2.8-.5.2-.3.2-.6,0-1l-.9-1.9h1.7l1.4,2.8c0,.2.2.3.4.4.2,0,.3.1.5.1.4,0,.6-.2.8-.5.2-.3.2-.6,0-1l-.9-1.9h1.7l1.4,2.8c0,.2.2.3.4.4s.3.1.5.1c.4,0,.6-.2.8-.5.2-.3.2-.6,0-1l-.9-1.9h3.3c.5,0,.8.2,1.2.5.3.3.5.7.5,1.2v11.1c0,.4-.2.8-.5,1.2-.3.3-.7.5-1.2.5H2.8c-.4,0-.8-.2-1.2-.5-.3-.3-.5-.7-.5-1.2V4.4c0-.4.2-.8.5-1.2.3-.3.7-.5,1.2-.5h1.1ZM3.2,8v7.4s13.6,0,13.6,0v-7.4s-13.6,0-13.6,0ZM3.2,8v7.4s0-7.4,0-7.4Z"
		/>
	</svg>
{/snippet}

{#snippet folderIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		stroke="none"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="currentColor"
	>
		<path
			class="st0"
			d="M3.5,16.5c-.4,0-.8-.1-1.1-.4-.3-.3-.4-.6-.4-1.1V4.5c0-.4.1-.8.4-1.1.3-.3.6-.4,1.1-.4h3.9c.2,0,.4,0,.6.1.2,0,.3.2.5.3l1.6,1.8h6.5c.4,0,.8.1,1.1.4.3.3.4.6.4,1.1v8.3c0,.4-.1.8-.4,1.1-.3.3-.6.4-1.1.4H3.5ZM3.8,14.7h12.4v-7.8s-6.8,0-6.8,0l-2-2.1h-3.7v9.9Z"
		/>
	</svg>
{/snippet}

{#snippet forwardArrowIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path d="M560-880 960-480l-400 400-71-71 329-329-329-329 71-71Z" />
	</svg>
{/snippet}

{#snippet forwardIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 240 240"
		fill="currentColor"
	>
		<path
			d="M211.18,143.13l-80.42,46.43c-17.8,10.28-40.06-2.57-40.06-23.13v-92.86c0-20.56,22.25-33.4,40.06-23.13l80.42,46.43c17.8,10.28,17.8,35.97,0,46.25Z"
		/><rect x="12.83" y="46" width="45.61" height="148" rx="22.8" ry="22.8" />
	</svg>
{/snippet}

{#snippet handIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M20,14 L20,17 C20,19.209139 18.209139,21 16,21 L10.0216594,21 C8.75045497,21 7.55493392,20.3957659 6.80103128,19.3722467 L3.34541668,14.6808081 C2.81508416,13.9608139 2.94777982,12.950548 3.64605479,12.391928 C4.35756041,11.8227235 5.38335813,11.8798792 6.02722571,12.5246028 L8,14.5 L8,13 L8.00393081,13 L8,11 L8.0174523,6.5 C8.0174523,5.67157288 8.68902517,5 9.5174523,5 C10.3458794,5 11.0174523,5.67157288 11.0174523,6.5 L11.0174523,11 L11.0174523,4.5 C11.0174523,3.67157288 11.6890252,3 12.5174523,3 C13.3458794,3 14.0174523,3.67157288 14.0174523,4.5 L14.0174523,11 L14.0174523,5.5 C14.0174523,4.67157288 14.6890252,4 15.5174523,4 C16.3458794,4 17.0174523,4.67157288 17.0174523,5.5 L17.0174523,11 L17.0174523,7.5 C17.0174523,6.67157288 17.6890252,6 18.5174523,6 C19.3458794,6 20.0174523,6.67157288 20.0174523,7.5 L20.0058962,14 L20,14 Z"
		/>
	</svg>
{/snippet}

{#snippet helpIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 48 48"
		stroke="currentColor"
		stroke-width="4"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M24.02,26.95c0-3.48,2.74-4.06,3.68-5,.95-.94,1.53-2.25,1.53-3.69,0-2.88-2.33-5.21-5.21-5.21-1.56,0-2.95.68-3.91,1.76-.44.5-.79,1.09-1.01,1.73"
		/><line x1="24" y1="34" x2="24" y2="34" /><circle cx="24" cy="24" r="20" />
	</svg>
{/snippet}

{#snippet imageIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm0-80h560v-560H200v560Zm40-80h480L570-480 450-320l-90-120-120 160Zm-40 80v-560 560Z"
		/>
	</svg>
{/snippet}

{#snippet importIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12,4 L12,17" /> <polyline points="7 8 12 3 17 8" /> <path d="M20,21 L4,21" />
	</svg>
{/snippet}

{#snippet infoIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12,12 L12,15" /> <line x1="12" y1="9" x2="12" y2="9" />
		<circle cx="12" cy="12" r="10" />
	</svg>
{/snippet}

{#snippet inPointIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polyline class="cls-1" points="15.43 21 5.73 21 5.73 3 15.43 3" /><polyline
			class="cls-1"
			points="13.45 7.65 17.8 12 13.45 16.35 13.45 16.35"
		/>
	</svg>
{/snippet}

{#snippet justifyLeftIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M6 10L14 10M6 6L18 6M6 14L18 14M6 18L14 18" />
	</svg>
{/snippet}

{#snippet justifyCenterIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M8 10L16 10M6 6L18 6M6 14L18 14M8 18L16 18" />
	</svg>
{/snippet}

{#snippet justifyRightIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M10 10h8M6 6h12M6 14h12m-8 4h8" />
	</svg>
{/snippet}

{#snippet keyframeIcon(className: string, active = false)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		fill="currentColor"
	>
		{#if active}
			<path class="cls-1" d="M10,1.54L1.3,10.24l8.7,8.7,8.7-8.7L10,1.54Z" />
		{:else}
			<path
				class="cls-1"
				d="M10,1.54L1.3,10.24l8.7,8.7,8.7-8.7L10,1.54ZM6.39,10.24l3.61-3.61,3.61,3.61-3.61,3.61-3.61-3.61Z"
			/>
		{/if}
	</svg>
{/snippet}

{#snippet linkIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M10.5,15.5 C10.5,14.1666667 10.5,13.5 10.5,13.5 C10.5,10.7385763 8.26142375,8.5 5.5,8.5 C2.73857625,8.5 0.5,10.7385763 0.5,13.5 C0.5,13.5 0.5,14.1666667 0.5,15.5"
			transform="rotate(-90 5.5 12)"
		/> <path d="M8,12 L16,12" />
		<path
			d="M23.5,15.5 C23.5,14.1666667 23.5,13.5 23.5,13.5 C23.5,10.7385763 21.2614237,8.5 18.5,8.5 C15.7385763,8.5 13.5,10.7385763 13.5,13.5 C13.5,13.5 13.5,14.1666667 13.5,15.5"
			transform="rotate(90 18.5 12)"
		/>
	</svg>
{/snippet}

{#snippet logoIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 283.8 439.33"
		fill="currentColor"
	>
		<path
			d="M205.18,4.25h-126.57C38.77,4.25,6.47,36.56,6.47,76.4v129.35c0,33.88,8.16,67.27,23.78,97.33l47.62,91.63c26.94,51.84,101.11,51.84,128.04,0l47.65-91.73c15.61-30.06,23.77-63.43,23.77-97.3V76.4c0-39.84-32.3-72.15-72.15-72.15ZM218.34,120.51c0,22.58-5.43,44.3-15.84,64.34l-24.47,47.1c-15.2,29.26-57.07,29.27-72.27,0l-24.45-47.05c-10.42-20.04-15.85-41.77-15.85-64.35v-12.61c0-22.49,18.23-40.72,40.72-40.72h71.44c22.49,0,40.72,18.23,40.72,40.72v12.56Z"
		/>
	</svg>
{/snippet}

{#snippet mouseIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M5 10L12 10M19 10L12 10M12 10L12 2.5" /><path
			d="M5 9C5 5.13401 8.13401 2 12 2V2C15.866 2 19 5.13401 19 9V15C19 18.866 15.866 22 12 22V22C8.13401 22 5 18.866 5 15V9Z"
		/>
	</svg>
{/snippet}

{#snippet moveIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12 5L12 19" />
		<path d="M9 17L12 20L15 17" />
		<path d="M15 7L12 4L9 7" />
		<path d="M5 12L19 12" />
		<path d="M17 15L20 12L17 9" />
		<path d="M7 9L4 12L7 15" />
	</svg>
{/snippet}

{#snippet opacityIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="M253.5-212Q160-304 160-436q0-65 25-121.5T254-658l226-222 226 222q44 44 69 100.5T800-436q0 132-93.5 224T480-120q-133 0-226.5-92ZM242-400h474q12-72-13.5-123T650-600L480-768 310-600q-27 26-53 77t-15 123Z"
		/>
	</svg>
{/snippet}

{#snippet outPointIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polyline class="cls-1" points="8.1 3 17.8 3 17.8 21 8.1 21" /><polyline
			class="cls-1"
			points="10.07 16.35 5.73 12 10.07 7.65 10.07 7.65"
		/>
	</svg>
{/snippet}

{#snippet paletteIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 -960 960 960"
		fill="currentColor"
	>
		<path
			d="m717.13-430 91 91v176.13q0 37.78-26.61 64.39t-64.39 26.61H162.87q-37.78 0-64.39-26.61t-26.61-64.39v-554.26q0-37.78 26.61-64.39t64.39-26.61H380q-9.91 19.91-15.23 44.12-5.31 24.21-6.84 46.88H162.87v554.26h554.26V-430Zm58.94-172.8 130.86 130.87-58.86 58.86L717.2-543.93q-21 12-45.6 19.76-24.6 7.76-51.6 7.76-76.49 0-130.04-53.54-53.55-53.53-53.55-130.01 0-76.47 53.54-130.05 53.53-53.58 130.01-53.58 76.47 0 130.05 53.55 53.58 53.55 53.58 130.04 0 27-7.76 51.6t-19.76 45.6ZM620-600q42 0 71-29t29-71q0-42-29-71t-71-29q-42 0-71 29t-29 71q0 42 29 71t71 29ZM162.87-430v267.13-554.26V-436.41v-12V-430Z"
		/>
	</svg>
{/snippet}

{#snippet pauseIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 240 240"
		fill="currentColor"
	>
		<rect x="21.71" y="12.01" width="72.27" height="218.54" rx="35.8" ry="35.8" /><rect
			x="145.38"
			y="12.01"
			width="72.27"
			height="218.54"
			rx="36.13"
			ry="36.13"
		/>
	</svg>
{/snippet}

{#snippet playIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 240 240"
		fill="currentColor"
	>
		<path
			d="M203.5,148.44l-134.43,77.62c-21.04,12.15-47.35-3.04-47.35-27.34V43.48c0-24.3,26.31-39.49,47.35-27.34l134.43,77.62c21.04,12.15,21.04,42.53,0,54.68Z"
		/>
	</svg>
{/snippet}

{#snippet pointerIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polygon points="7 20 7 4 19 16 12 16 7 21" />
	</svg>
{/snippet}

{#snippet presetsIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		fill="currentColor"
	>
		<path
			class="st0"
			d="M7.6,10.5c.5,0,.8.2,1.2.5.3.3.5.7.5,1.2v4.4c0,.4-.2.8-.5,1.2-.3.3-.7.5-1.2.5H3.2c-.4,0-.8-.2-1.2-.5-.3-.3-.5-.7-.5-1.2v-4.4c0-.4.2-.8.5-1.2.3-.3.7-.5,1.2-.5h4.5ZM3.4,12.3v4.2h4s0-4.1,0-4.1H3.4Z"
		/>
		<path
			class="st0"
			d="M16.8,10.6c.5,0,.8.2,1.2.5.3.3.5.7.5,1.2v4.4c0,.4-.2.8-.5,1.2-.3.3-.7.5-1.2.5h-4.5c-.4,0-.8-.2-1.2-.5-.3-.3-.5-.7-.5-1.2v-4.4c0-.4.2-.8.5-1.2.3-.3.7-.5,1.2-.5h4.5ZM12.6,12.3v4.3h4s0-4.2,0-4.2h-4Z"
		/>
		<path
			class="st0"
			d="M7.6,1.4c.5,0,.8.2,1.2.5.3.3.5.7.5,1.2v4.4c0,.4-.2.8-.5,1.2-.3.3-.7.5-1.2.5H3.2c-.4,0-.8-.2-1.2-.5-.3-.3-.5-.7-.5-1.2V3c0-.4.2-.8.5-1.2.3-.3.7-.5,1.2-.5h4.5ZM3.4,3.1v4.2h4s0-4.2,0-4.2H3.4Z"
		/>
		<path
			class="st0"
			d="M16.8,1.4c.5,0,.8.2,1.2.5.3.3.5.7.5,1.2v4.4c0,.4-.2.8-.5,1.2-.3.3-.7.5-1.2.5h-4.5c-.4,0-.8-.2-1.2-.5-.3-.3-.5-.7-.5-1.2V3c0-.4.2-.8.5-1.2.3-.3.7-.5,1.2-.5h4.5ZM12.6,3.1v4.3h4s0-4.2,0-4.2h-4Z"
		/>
	</svg>
{/snippet}

{#snippet redoIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M19,17 L19,15 C19,10.0294373 15.1944204,6 10.5,6 C5.80557963,6 2,10.0294373 2,15" />
		<polyline points="16 15 19 18 22 15" />
	</svg>
{/snippet}

{#snippet scaleToFillIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polyline class="cls-1" points="2.87 11.96 2.87 17.09 8.11 17.09" /><line
			class="cls-1"
			x1="8.39"
			y1="11.68"
			x2="2.87"
			y2="17.09"
		/><polyline class="cls-1" points="16.69 8.5 16.69 3.37 11.45 3.37" /><line
			class="cls-1"
			x1="11.2"
			y1="8.75"
			x2="16.69"
			y2="3.37"
		/>
	</svg>
{/snippet}

{#snippet scaleToFitIcon(className: string)}
	<svg
		class={className}
		role="img"
		id="Layer_1"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 20 20"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polyline class="cls-1" points="7.91 17.3 7.91 12.16 2.66 12.16" /><line
			class="cls-1"
			x1="2.38"
			y1="17.57"
			x2="7.91"
			y2="12.16"
		/><polyline class="cls-1" points="11.53 3.29 11.53 8.42 16.77 8.42" /><line
			class="cls-1"
			x1="17.02"
			y1="3.04"
			x2="11.53"
			y2="8.42"
		/>
	</svg>
{/snippet}

{#snippet scissorsIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M3 18L15 9M15 15L3 6" /> <circle cx="18" cy="7" r="3" />
		<circle cx="18" cy="17" r="3" />
	</svg>
{/snippet}

{#snippet seekIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M6.58748441 4.80875867C4.40860039 6.45129836 3 9.06115105 3 12 3 16.9705627 7.02943725 21 12 21 16.9705627 21 21 16.9705627 21 12 21 7.02943725 16.9705627 3 12 3L12 7M12 13L8 9"
		/>
	</svg>
{/snippet}

{#snippet settingsIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M5.03506429,12.7050339 C5.01187484,12.4731696 5,12.2379716 5,12 C5,11.7620284 5.01187484,11.5268304 5.03506429,11.2949661 L3.20577137,9.23205081 L5.20577137,5.76794919 L7.9069713,6.32070904 C8.28729123,6.0461342 8.69629298,5.80882212 9.12862533,5.61412402 L10,3 L14,3 L14.8713747,5.61412402 C15.303707,5.80882212 15.7127088,6.0461342 16.0930287,6.32070904 L18.7942286,5.76794919 L20.7942286,9.23205081 L18.9649357,11.2949661 C18.9881252,11.5268304 19,11.7620284 19,12 C19,12.2379716 18.9881252,12.4731696 18.9649357,12.7050339 L20.7942286,14.7679492 L18.7942286,18.2320508 L16.0930287,17.679291 C15.7127088,17.9538658 15.303707,18.1911779 14.8713747,18.385876 L14,21 L10,21 L9.12862533,18.385876 C8.69629298,18.1911779 8.28729123,17.9538658 7.9069713,17.679291 L5.20577137,18.2320508 L3.20577137,14.7679492 L5.03506429,12.7050339 Z"
		/> <circle cx="12" cy="12" r="1" />
	</svg>
{/snippet}

{#snippet speakerIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M13 5v14l-5-4H3V9h5z" />
		<path stroke-linecap="round" d="M13 14c1.5-1 1.5-3 0-4" />
		<path
			d="M16 16C18.0858253 13.9141747 18.0858253 10.0858253 16 8M18 19C21.98552 15.01448 22.0076803 9.00768033 18 5"
		/>
	</svg>
{/snippet}

{#snippet spinningIcon(className: string)}
	<svg
		class={['animate-spin', className]}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 64 64"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M32 3C35.8083 3 39.5794 3.75011 43.0978 5.20749C46.6163 6.66488 49.8132 8.80101 52.5061 11.4939C55.199 14.1868 57.3351 17.3837 58.7925 20.9022C60.2499 24.4206 61 28.1917 61 32C61 35.8083 60.2499 39.5794 58.7925 43.0978C57.3351 46.6163 55.199 49.8132 52.5061 52.5061C49.8132 55.199 46.6163 57.3351 43.0978 58.7925C39.5794 60.2499 35.8083 61 32 61C28.1917 61 24.4206 60.2499 20.9022 58.7925C17.3837 57.3351 14.1868 55.199 11.4939 52.5061C8.801 49.8132 6.66487 46.6163 5.20749 43.0978C3.7501 39.5794 3 35.8083 3 32C3 28.1917 3.75011 24.4206 5.2075 20.9022C6.66489 17.3837 8.80101 14.1868 11.4939 11.4939C14.1868 8.80099 17.3838 6.66487 20.9022 5.20749C24.4206 3.7501 28.1917 3 32 3L32 3Z"
			stroke="currentColor"
			stroke-width="8"
			stroke-linecap="round"
			stroke-linejoin="round"
		></path><path
			d="M32 3C36.5778 3 41.0906 4.08374 45.1692 6.16256C49.2477 8.24138 52.7762 11.2562 55.466 14.9605C58.1558 18.6647 59.9304 22.9531 60.6448 27.4748C61.3591 31.9965 60.9928 36.6232 59.5759 40.9762"
			stroke="currentColor"
			stroke-width="15"
			stroke-linecap="round"
			stroke-linejoin="round"
			class="text-zinc-900"
		></path>
	</svg>
{/snippet}

{#snippet testIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<polygon points="20 8 20 20 4 20 4 8" />
		<polyline stroke-linejoin="round" points="8 4 12 7.917 16 4" />
	</svg>
{/snippet}

{#snippet textIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2.5"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M4 8L5 4H12M20 8L19 4H12M12 4V20M12 20H8M12 20H16" />
	</svg>
{/snippet}

{#snippet undoIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M5,17 L5,15 C5,10.0294373 8.80557963,6 13.5,6 C18.1944204,6 22,10.0294373 22,15" />
		<polyline points="8 15 5 18 2 15" />
	</svg>
{/snippet}

{#snippet warningIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M12 8L12 13" /> <line x1="12" y1="16" x2="12" y2="16" />
		<circle cx="12" cy="12" r="10" />
	</svg>
{/snippet}

{#snippet zoomInIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path d="M4,20 L9.58788778,14.4121122" /><path
			d="M14,16 C10.6862915,16 8,13.3137085 8,10 C8,6.6862915 10.6862915,4 14,4 C17.3137085,4 20,6.6862915 20,10 C20,13.3137085 17.3137085,16 14,16 Z"
		/> <path d="M16.6666667 10L11.3333333 10M14 7.33333333L14 12.6666667" />
	</svg>
{/snippet}

{#snippet zoomOutIcon(className: string)}
	<svg
		class={className}
		role="img"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		stroke="currentColor"
		stroke-width="2"
		stroke-linecap="round"
		stroke-linejoin="round"
		fill="none"
	>
		<path
			d="M14,16 C10.6862915,16 8,13.3137085 8,10 C8,6.6862915 10.6862915,4 14,4 C17.3137085,4 20,6.6862915 20,10 C20,13.3137085 17.3137085,16 14,16 Z"
		/> <path d="M16.6666667 10L11.3333333 10M4 20L9.58788778 14.4121122" />
	</svg>
{/snippet}

<script lang="ts">
	...
	let barSize = $derived.by(() => {
		const barStartNormalised = invLerp(min, max, barStart);
		const clampedValue = Math.min(Math.max(normalisedValue, 0), 1);
...
<div
	...
	class={[!vertical ? 'w-full' : 'h-full', 'touch-none select-none group relative']}
>
	...
	<div
		...
		onmousedown={(e) => {
			...
			const normalised = Math.min(Math.max(normalisedSize, 0), 1);
...
<svelte:window
	onmousemove={(e) => {
		...
		throttle(() => {
			...
			let normalised;
			if (!vertical) {
				normalised = Math.min(Math.max(position / width, 0), 1);
			} else {
				normalised = 1 - Math.min(Math.max(position / height, 0), 1);
			}
			const lerped = lerp(min, max, normalised);
			value = Math.round(lerped * 100) / 100;
			valueChanged();
